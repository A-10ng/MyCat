 ...
 */
public class CatFoodView extends FrameLayout {
    private static final int WHAT_ADD_PROGRESS = 1;
    /**
     * view变化的y抖动范围
     */
    private static final int CHANGE_RANGE = 15;
    /**
     ...
     * 添加水滴时动画显示view执行的时间
     */
    public static final int ANIMATION_SHOW_VIEW_DURATION = 1000;
    /**
     * 控制水滴动画的快慢
     */
    private List<Float> mSpds = Arrays.asList(0.4f, 0.3f, 0.2f, 0.1f);
    /**
     * x最多可选取的随机数值
     */
    ...
    private static final List<Float> Y_MAX_CHOSE_RANDOMS = Arrays.asList(
            0.2f,0.22f,0.24f,0.26f,0.28f,0.3f);
    /**
     * x坐标当前可选的随机数组
     */
    private List<Float> mXCurrentCanChoseRandoms = new ArrayList<>();
    /**
     * y坐标当前可选的随机数组
     */
    private List<Float> mYCurrentCanChoseRandoms = new ArrayList<>();

    /**
     * 已经选取x的随机数值
     */
    private List<Float> mXChosedRandoms = new ArrayList<>();
    /**
     * 已经选取y的随机数值
     */
    private List<Float> mYChosedRandoms = new ArrayList<>();


    private Random mRandom = new Random();
    private List<View> mViews = new ArrayList<>();
    private int mChildViewRes = R.layout.item_catfood;//子view的资源文件
    ...
    public CatFoodView( Context context) {
        this(context, null);
    }

    public CatFoodView( Context context,  AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CatFoodView( Context context,  AttributeSet attrs, int defStyleAttr) {
    ...
    private void reset() {
        ...
        mViews.clear();
        mXChosedRandoms.clear();
        mYChosedRandoms.clear();
        mYCurrentCanChoseRandoms.clear();
        mXCurrentCanChoseRandoms.clear();
     ...
     * @param waters
     */
    public void setWaters(final List<CatFood> waters) {
     ...
     * @param waters
     */
    private void setDatas(List<CatFood> waters) {
        reset();
    ...
    private void setCurrentCanChoseRandoms() {
        mXCurrentCanChoseRandoms.addAll(X_MAX_CHOSE_RANDOMS);
        mYCurrentCanChoseRandoms.addAll(Y_MAX_CHOSE_RANDOMS);
    }

    /**
     * 添加水滴view
     */
    private void addWaterView(List<CatFood> waters) {
        for (int i = 0; i < waters.size(); i++) {
            final CatFood water = waters.get(i);
    ...
    private void handViewClick(View view) {
        ...
        if (tag instanceof CatFood) {
            CatFood waterTag = (CatFood) tag;
            mTotalConsumeWater += waterTag.getNumber();
            Toast.makeText(getContext(), "当前点击的是：" + waterTag.getType() + "水滴的值是:"
    ...
    private void setChildViewLocation(View view) {
        view.setX((float) (maxX * getX_YRandom(mXCurrentCanChoseRandoms, mXChosedRandoms)));
        view.setY((float) (maxY * getX_YRandom(mYCurrentCanChoseRandoms, mYChosedRandoms)));
        view.setTag(R.string.original_y, view.getY());
    }
